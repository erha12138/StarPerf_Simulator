...
def env_agents_config(cfg):
    envs = [NormalizedActions(gym.make(env_name)) for env_name in cfg.envs]
...
def manage_agents(cfg, envs, agents, add_agents=0, remove_indices=[]):
    ...
    for _ in range(add_agents):
        ...
        new_env = NormalizedActions(gym.make(new_env_name))
...
def train(cfg, envs, agents):
    ...
    for i_ep in range(cfg.train_eps):
        if i_ep % 10 == 0 and i_ep > 0:  # 每10个回合动态调整智能体数量
